...
| `DOLPHIN_USER_DIR` | `$XDG_DATA_HOME/dolphin-emu` | Dolphin's data directory (`StateSaves/`, `GC/`, `Wii/`). |
| `DOLPHIN_CONFIG_DIR` | `$XDG_CONFIG_HOME/dolphin-emu` | Dolphin's INI directory, shared with the desktop launcher so a pad rebound in a desktop session applies to broker launches too. |
