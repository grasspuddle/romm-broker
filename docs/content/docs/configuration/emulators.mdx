...
| `DOLPHIN_USER_DIR` | `/config/.local/share/dolphin-emu` | Dolphin's data directory (`StateSaves/`, `GC/`, `Wii/`). Mirrors Dolphin's own XDG default; the broker passes no `-u`, so change this only if `$XDG_DATA_HOME` moves. |
| `DOLPHIN_CONFIG_DIR` | `/config/.config/dolphin-emu` | Dolphin's INI directory, shared with the desktop launcher so a pad rebound in a desktop session applies to broker launches too. |
